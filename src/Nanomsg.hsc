...
data Socket a = Socket a Libnanomsg.Socket
    deriving (Eq, Show)

-- | Typeclass for all sockets
class SocketType a where
    socketType :: a -> Libnanomsg.Protocol

instance SocketType Pair where
    socketType Pair = Libnanomsg.protocolPair

instance SocketType Req where
    socketType Req = Libnanomsg.protocolReq

instance SocketType Rep where
    socketType Rep = Libnanomsg.protocolRep

instance SocketType Pub where
    socketType Pub = Libnanomsg.protocolPub

instance SocketType Sub where
    socketType Sub = Libnanomsg.protocolSub

instance SocketType Surveyor where
    socketType Surveyor = Libnanomsg.protocolSurveyor

instance SocketType Respondent where
    socketType Respondent = Libnanomsg.protocolRespondent

instance SocketType Push where
    socketType Push = Libnanomsg.protocolPush

instance SocketType Pull where
    socketType Pull = Libnanomsg.protocolPull
...
throwErrno' :: String -> Errno -> IO a
throwErrno' loc (Errno errno) = do
    errCString <- c_nn_strerror errno
    errString <- peekCString errCString
    throwIO $ NNException $ printf "nanomsg-haskell error at %s. Errno %d: %s" loc (fromIntegral errno :: Int) errString
